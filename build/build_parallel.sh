...
echo 'Start checking kernel prebuilt list'
BASE_DIR="device/qcom/${TARGET_BOARD_PLATFORM}-kernel"
# Baseline golden list
KERNEL_PREBUILT_GOLDEN_BASE=(
    "debug/kernel-tests/"
    "dtbs/"
    "host/"
    "kernel-abl/abl.bin"
    "kernel-abl/bootaa64.efi"
    "kernel-abl/LinuxLoader.efi"
    "kernel-abl/abl-userdebug/unsigned_abl.elf"
    "kernel-gbl/"
    "kernel-headers/"
    "kernel-uapi-headers/"
    "kp-dtbs/"
...
# Target-specific overlay lists
KERNEL_PREBUILT_GOLDEN_ADD=()
KERNEL_PREBUILT_GOLDEN_REMOVE=()
case "$TARGET_BOARD_PLATFORM" in
  art)
    # art adds 16k/
    KERNEL_PREBUILT_GOLDEN_ADD+=("16k/")
    ;;
  canoe)
    # canoe uses baseline list
    ;;
esac
# Compose final golden list with remove then add (exact string match)
KERNEL_PREBUILT_GOLDEN_LIST=("${KERNEL_PREBUILT_GOLDEN_BASE[@]}")
if [ "${#KERNEL_PREBUILT_GOLDEN_REMOVE[@]}" -gt 0 ]; then
  tmp_list=()
  for item in "${KERNEL_PREBUILT_GOLDEN_LIST[@]}"; do
    to_remove=false
    for rem in "${KERNEL_PREBUILT_GOLDEN_REMOVE[@]}"; do
      if [[ "$item" == "$rem" ]]; then
        to_remove=true
        break
      fi
  ...
  KERNEL_PREBUILT_GOLDEN_LIST=("${tmp_list[@]}")
fi
if [ "${#KERNEL_PREBUILT_GOLDEN_ADD[@]}" -gt 0 ]; then
  KERNEL_PREBUILT_GOLDEN_LIST+=("${KERNEL_PREBUILT_GOLDEN_ADD[@]}")
fi
...
while IFS= read -r rel_path; do
    ...
    matched=false
    for pattern in "${KERNEL_PREBUILT_GOLDEN_LIST[@]}"; do
...
done < <(find "$BASE_DIR" -mindepth 1 -printf "%P\n")
...
if [[ ${#errors[@]} -gt 0 || ${#missing[@]} -gt 0 ]]; then
    echo "-----------------------------------------------------"
    echo "cnt_kernel_prebuilt_errors : ${#errors[@]}"
    echo "cnt_kernel_prebuilt_missing : ${#missing[@]}"
    echo "Error: Found files under $BASE_DIR not matching allowed prebuilt pattern."
    for e in "${errors[@]}"; do
        echo "    Unexpected: $e   # Not in allowed list"
    done
    for m in "${missing[@]}"; do
        echo "    Missing: $m      # Expected but not found"
    done
    echo "-----------------------------------------------------"
    exit 1
fi

echo 'Check kernel prebuilt list finished'
